...
#We use the image dataset to train the generator and the discriminator. The generator will generate handwritten digits resembling the image data.
...
# Use `@` annotation so the function can be compiled.
@tf.function
...
## Train the model
#We call train() method defined above to train the generator and discriminator simultaneously. 
...
%%time
train(train_dataset, EPOCHS) #set to 50
...
#download the animation e.g. from google cloud:
try:
